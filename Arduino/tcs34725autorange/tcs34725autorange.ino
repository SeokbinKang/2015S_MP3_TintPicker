...
void loop(void) {
  ...
  if(lightStatus==0) rgb_sensor.tcs.setInterrupt(true);
    else rgb_sensor.tcs.setInterrupt(false);
  rgb_sensor.getData();
  ble_do_events();
  
  int buttonState,buttonState2,buttonState3;
  ...
  int tone_;
  int r,g,b,c;
  r=map(rgb_sensor.r_comp,1000, 20000, 1,254);
  g=map(rgb_sensor.g_comp,1000, 20000, 1,254);
  b=map(rgb_sensor.b_comp,1000, 20000, 1,254);
  c=map(rgb_sensor.c_comp,1000, 20000, 1,254);
  tone_=r+g+b;
  tone(10,tone_,200);  
  sprintf(tt,"RGB %d %d %d %d\n",r,g,b,c);

  Serial.println(tt);
   sendBluetoothData(tt);
   delay(1000);
}

void sendBluetoothData(char q[]) {
  String t(q);
  int len=t.length();
   if ( ble_connected() )
  {
